...
So let's set it up properly. Real domain, real certificates, local storage on a path you control. First though, a quick word on what this thing is, because it's not just another Nextcloud clone.
...
For a homelab that's a big deal. Your backup is a copy of one directory, full stop. It also runs comfortably on low-power hardware, where Nextcloud with a handful of apps enabled wants 1 to 2 GB of RAM before it feels responsive.
...
Out of the box, search only looks at filenames. Apache Tika adds search inside your documents, which on a file server is the difference between "I know I saved that thing somewhere" and finding it.

Add it to your `COMPOSE_FILE` alongside whichever external-proxy file you settled on earlier:

```bash
COMPOSE_FILE=docker-compose.yml:external-proxy/opencloud-exposed.yml:search/tika.yml
```
...
Do note that indexing happens as files get uploaded or changed. Anything already sitting in your storage from before you enabled Tika won't have its contents indexed until you touch it again, so don't panic when an old document doesn't show up.
...
Add a DNS record for `euro-office.example.com`, which your wildcard already covers, then:

```bash
COMPOSE_FILE=docker-compose.yml:external-proxy/opencloud-exposed.yml:weboffice/euro-office.yml:external-proxy/euro-office-exposed.yml
EURO_OFFICE_DOMAIN=euro-office.example.com
```

Do note the `-exposed` on that second file. Just like OpenCloud, `external-proxy/euro-office.yml` binds to `127.0.0.1:9900` and only works if NPM is on the same machine. Match whichever variant you picked earlier or you'll be troubleshooting a 502 for no reason.
...
Give it a minute before you test. The healthcheck allows a 120 second start period and it genuinely uses most of that on a cold start, so don't panic when the container sits there looking unhealthy. Once it settles, create a new `.docx` from inside OpenCloud. It should open right in the browser, and there we go.

One thing worth knowing. The `.env.example` file has a `EURO_OFFICE_JWT_SECRET` and the README tells you to change it for production, but on the version of the repo I'm running that variable isn't referenced by any compose file, so setting it doesn't do anything. Not a hole by itself, since the document server is only reachable through your proxy, but don't assume it's protecting you.
...
```bash
COMPOSE_FILE=docker-compose.yml:external-proxy/opencloud-exposed.yml:radicale/radicale.yml
```
...
```bash
COMPOSE_FILE=docker-compose.yml:external-proxy/opencloud-exposed.yml:antivirus/clamav.yml
...
Back those up and you've backed up OpenCloud. There's no database to dump, and nothing that has to stay in sync with anything else. The simplest possible version:
...
If you want something better than an rsync to a spare drive, I wrote a [full Proxmox Backup Server guide](/proxmox-backup-server-complete-guide) that covers backing up Docker hosts with the PBS client and pushing a copy offsite. That's what I'd point you at for anything you care about.
...
The no-database thing isn't just a talking point. It changes how this feels to run day to day. Spaces are a better sharing model than folder-based sharing. It's fast, it's light, and this compose repo is one of the cleaner modular setups I've worked with, which counts for a lot when you're the one maintaining it.
