...
### What About Document Editing?

OpenCloud does do in-browser document editing through Collabora Online, and everything for it lives in `weboffice/collabora.yml` in the repo.

I'm not walking through it here though, because it's a lot more than one extra line in `COMPOSE_FILE`. It wants its own subdomain and proxy host, a one-shot service to generate the WOPI proof key, host font mounts so your documents don't render in the wrong typeface, and some fiddly SSL settings that change depending on whether your proxy is terminating TLS. That's its own guide and I'd rather do it properly than hand you half of it, so I'll link it here once it's written.

If you do go poking at it, do note that Collabora and Euro Office are mutually exclusive. They both use the same WOPI service, so pick one.
