...
Append `search/tika.yml` to your `COMPOSE_FILE` line, then force OpenCloud to be recreated. This part matters: a Tika container can be running and healthy while an older OpenCloud container is still missing the extractor settings.

```bash
docker compose up -d --force-recreate opencloud
```

The compose file wires everything up for you: it sets the extractor type to Tika, points OpenCloud at `http://tika:9998`, turns on `FRONTEND_FULL_TEXT_SEARCH_ENABLED`, and adds a healthcheck so OpenCloud waits for Tika to come up before it starts. Verify those settings made it into the live container:

```bash
docker compose exec opencloud env | grep -E '^(SEARCH_EXTRACTOR|FRONTEND_FULL_TEXT)'
```

You should see `SEARCH_EXTRACTOR_TYPE=tika`, the Tika URL, and `FRONTEND_FULL_TEXT_SEARCH_ENABLED=true`. If that command prints nothing, `search/tika.yml` is not part of the active `COMPOSE_FILE`, or OpenCloud was not recreated after you added it.
