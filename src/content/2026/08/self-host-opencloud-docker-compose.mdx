...
If you want to open a `.docx` in the browser and edit it right there, you need Collabora:
...
`COLLABORA_SSL_ENABLE=false` is correct here because NPM is terminating TLS and talking plain HTTP to Collabora. You should also comment out `COLLABORA_SSL_VERIFICATION=false` in `.env` so Collabora verifies the certs it talks to.
